...
@pytest.mark.parametrize(
    "include_value",
    [
        "queries.yml",  # bare string
        "false",  # YAML bool false
        "0",  # YAML int 0
        '""',  # empty string
    ],
)
def test_include_non_list_raises(tmp_path: Path, include_value: str) -> None:
    main = tmp_path / "laketower.yml"
    main.write_text(f"include: {include_value}\n")

    with pytest.raises(TypeError, match="'include' must be a list"):
        config.load_yaml_config(main)


def test_include_empty_list(tmp_path: Path) -> None:
    main = tmp_path / "laketower.yml"
    main.write_text(
        yaml.dump(
            {"include": [], "tables": [{"name": "t", "uri": "p", "format": "delta"}]}
        )
    )
